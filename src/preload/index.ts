...
const api: IpcApi = withoutRetiredSaveApi({
  ...
  agents: {
    ...
    onPlanBadgesInvalidated: (callback) => {
      const listener = (_event: any, payload: PlanBadgesInvalidatedPayload) => callback(payload);
      ipcRenderer.on(PLAN_BADGES_INVALIDATED, listener);
      return () => ipcRenderer.removeListener(PLAN_BADGES_INVALIDATED, listener);
    },
  ...
  plans: {
    ...
    ledgerProjection: (planId) => ipcRenderer.invoke(PLAN_LEDGER_PROJECTION_CHANNEL, planId),
    factualRegister: (planId) => ipcRenderer.invoke(PLAN_FACTUAL_REGISTER_CHANNEL, planId),
