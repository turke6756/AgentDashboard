...
try {
  ...
  const defaultResult = queryLibrary(store, { query: 'needle', mode: 'keyword', limit: 1, highlight_doc_id: 'trusted' });
  ...
  const pdfResult = queryLibrary(store, { query: 'needle', mode: 'keyword', doc_ids: ['pdf-doc'], highlight_doc_id: 'pdf-doc' });
