...
import { listLibraryReportSources, type LibraryReportFile, type LibraryReportRoot, type LibraryReportSourcesInventory } from './library-report-sources';
...
function reportRelPath(root: LibraryReportRoot, file: LibraryReportFile): string { return `${REPORT_PREFIXES[root]}${file.rel_path}`; }
function isReportOwned(relPath: string): boolean { return Object.values(REPORT_PREFIXES).some((prefix) => portable(relPath).startsWith(prefix)); }
function selectRows(rows: readonly LibraryDocumentRow[], warn: (message: string) => void): Map<string, LibraryDocumentRow> {
  ...
  for (const row of rows) { const key = portable(row.source_rel_path); const group = grouped.get(key) ?? []; group.push(row); grouped.set(key, group); }
...
function statusFor(row: LibraryDocumentRow | undefined, diskSize: number, hash?: string): ShelfStatus {
  ...
  if (INDEXING_STATUSES.has(row.status as LibraryDocumentStatus)) return 'indexing'; if (row.status === 'error') return 'error'; return 'ready';
}
...
export async function listLibraryShelf(workspaceRoot: string, store: LibraryStore, options: ListLibraryShelfOptions = {}): Promise<ShelfRow[]> {
  ...
  const diskRows = listLibraryDocumentsByRelPaths(store, disk.map((item) => item.relPath));
  ...
  for (const [relPath, row] of selectedAll) {
    ...
    shelf.push({ ...row, status: row.status as LibraryDocumentStatus, shelf_status: row.status === 'error' ? 'error' : INDEXING_STATUSES.has(row.status as LibraryDocumentStatus) ? 'indexing' : 'ready' });
  }
