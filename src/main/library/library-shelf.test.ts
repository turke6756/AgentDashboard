...
test('disk reports left-join index rows, stamp folder identity, append user rows, and apply status precedence', async (t) => {
  const f = fixture(); t.after(f.close); upsertLibraryDocument(f.store, row('ready-inbox', '.lares/library/inbox/ready.md', 'same')); upsertLibraryDocument(f.store, row('stale-error', '.lares/library/cleared/stale.md', 'old!', { status: 'error' })); upsertLibraryDocument(f.store, row('error', '.lares/library/cleared/error.md', 'error', { status: 'error', attempt_count: 2 })); upsertLibraryDocument(f.store, row('indexing', '.lares/library/cleared/indexing.md', 'index', { status: 'embedding' })); upsertLibraryDocument(f.store, row('gone', '.lares/library/inbox/gone.md', 'gone')); upsertLibraryDocument(f.store, row('user-doc', '.lares/library/sources/manual.md', 'manual'));
  ...
  assert.equal(rows.find((item) => item.id === 'error')?.attempt_count, 2); assert.equal(rows.find((item) => item.id.startsWith('shelf:'))?.attempt_count, 0);
});
