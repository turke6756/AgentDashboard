...
test('production Windows codex launches receive their computed lane MCP toolset', async () => {
  ...
  const cases = [
    { id: 'codex-supervisor', flags: { isSupervisor: true }, toolsets: 'orchestration,comms,observability-core,plans,browser-present,checkpoints,memory,migration,library-read', model: null },
    { id: 'codex-worker', flags: { isSupervised: true }, toolsets: 'comms,observability-core,browser-present,plans-read,memory,checkpoints-read,library-read', model: null },
    { id: 'codex-researcher', flags: { isResearcher: true }, toolsets: 'browser,plans-read,library-read', model: RESEARCHER_CODEX_MODEL },
  ] as const;
...
test('production WSL codex researcher preserves MCP bytes on fresh launch and resume', async () => {
  ...
  assert.ok(resumeConfigs.includes('mcp_servers.agent-dashboard.env.DASHBOARD_MCP_TOOLSETS="browser,plans-read,library-read"'));
