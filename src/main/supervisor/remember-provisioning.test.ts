...
import { AgentSupervisor, REMEMBER_SKILL_V1_HASH, REMEMBER_SKILL_V2_HASH } from './index';
...
test('the Claude-supervisor remember copy is a managed v3 entry in SUPERVISOR_FILES', () => {
  ...
  assert.deepEqual(e.previousHashes, { 1: REMEMBER_SKILL_V1_HASH, 2: REMEMBER_SKILL_V2_HASH });
});

test('the Codex-supervisor remember copy is a managed v3 entry in SUPERVISOR_FILES_CODEX', () => {
...
test('the Claude-worker remember copy is a managed v3 entry in WORKER_FILES_CLAUDE', () => {
...
test('a real Codex-worker scaffold write lands the remember copy under .agents/skills/', () => {
  ...
  try {
    ...
    assert.equal(sidecar['workers/codex/.agents/skills/remember/SKILL.md'], 3);
  } finally {
