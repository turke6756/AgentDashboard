  ...
  return [
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          is_researcher: { type: 'boolean', description: 'Launch the workspace RESEARCHER role-lane (default: false) on Claude, Codex, or Antigravity (`agy`); Grok is unsupported. Researchers browse + research the web and write findings into .lares/library/inbox/. Claude launches add a native tool allowlist/denylist and a PreToolUse Write guard. Codex and agy researcher launches have no enforced write boundary. Every provider uses the user\'s normal provider home, including its settings and session history. When true, the app manages the provider-specific researcher cwd and browser toolset.' },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          plan_id:            { type: 'string', pattern: PLAN_ID_PATTERN, description: `${PLAN_ID_DESCRIPTION} Provide both plan_id and planning_intent_id for a plan-bound run, or omit both for a plan-less deliberation.` },
          planning_intent_id: { type: 'string', pattern: '^int_[0-9a-f]{8}$', description: 'The active same-plan PLAN-INTENT id served by this deliberation. Provide both plan_id and planning_intent_id for a plan-bound run, or omit both for a plan-less deliberation.' },
